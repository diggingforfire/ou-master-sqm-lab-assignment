...
private Figure indentedTree(loc project, str projectName, set[MethodsByFile] metric, str projectLevelMetricLabel, num projectLevelMetric) {
	...
	for (metricForFile <- take(3, sort(metric, sortFileByMetricValue))) {
		...
		showMoreMethodsText = text("      Show <itemGroupCount> more (<size(metricForFile.methods) - methodCount> remaining)", fontBold(true), fontItalic(true), left(), fontColor("Peru"), font("Consolas"), fontSize(8), onMouseDown(bool (int butnr, map[KeyModifier,bool] modifiers) { 
			...
			println(methodExpansionCount[tmpPath]);  
			return true;
		} ));
		rows += [showMoreMethodsText];
	} 
	
	return vcat(rows, vgap(5), halign(0.1));
}